...
printf "\n"
printf "Quick analysis of $FILE contract code...\n\n"
...
printf "\n"
printf "List line counts of each found modifier:\n"

awk '/^ *modifier/ { print $0 }' $FILE | sed 's/^.*modifier//' | sed 's/[(].*$//' | sed 's/[{}].*$//' | sed '/^[[:blank:]]*$/d' | while read -r line; do echo "  "$(cat $FILE | grep -c "$line") $(echo $line); done | sort -nr
